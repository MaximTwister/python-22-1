...
def start_gen_work():
    ...
    for gen in data:
        if gen["id"] == gen_id:
            ...
            gen["session_after_maint"]["start"] = time.time()
...
def stop_gen_work():
    ...
    for gen in data:
        if gen["id"] == gen_id:
            ...
            gen["session_after_maint"]["stop"] = time.time()
...
def calculate_motohours(gen):
    session_total_delta_seconds = gen["session"]["stop"] - gen["session"]["start"]
    gen["total_motohours"] += round(session_total_delta_seconds / 3600, 3)
    gen["session"] = EMPTY_SESSION
    session_after_maimt_delta_seconds = gen["session_after_maint"]["stop"] - gen["session_after_maint"]["start"]
    gen["motohours_after_maint"] += round(session_after_maimt_delta_seconds / 3600, 3)



def notify(gen):
    if gen["total_motohours"] >= gen["change_oil_period"]:
        gen["oil"] = MAINTENANCE
...
def add_gen():
    ...
    gen["id"] = get_id(data)
    gen["change_oil_period"] = float(gen["change_oil_period"])
    gen["session"] = EMPTY_SESSION
    gen["session_after_maint"] = EMPTY_SESSION_AFTER_MAINT
    gen["total_motohours"] = 0
    gen['motohours_after_maint'] = 0
    gen["state"] = STOPPED
    gen["oil"] = GOOD
...
def perform_maintenance():
    gen_id = select_gen_menu(filter_func=filter_maintainable_gens, state="oil")
    data = load_generators()
    for gen in data:
        if gen["id"] == gen_id:
            gen["oil"] = GOOD
